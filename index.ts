...
for (let i = 0; i < codes.length; i++) {
  ...
  // @ts-ignore
  const codeMirror = CodeMirror(editorNode, {
...
window.addEventListener("mousemove", (e) => {
  ...
  inputs.pointer.y = e.pageY - /*toGlobal*/ window.scrollY;
...
function render(now: number) {
  ...
  const editorSizeX = 600,
    editorSizeY = 400;
  const canvasSizeX = editorSizeX,
    canvasSizeY = editorSizeX / 2;
  const canvasRetinaSizeX = canvasSizeX * devicePixelRatio,
    canvasRetinaSizeY = canvasSizeY * devicePixelRatio;
  ...
  let canvasesTop = []; // TODO: implement this
  {
    ...
    for (let i = 0; i < editors.length; i++) {
      ...
      canvasesTop.push(
        i < 4
          ? playgroundGap
          : playgroundGap + canvasSizeY + editorSizeY + playgroundGap
      );
  ...
  let newLastPointer;
  {
    ...
    if (inputs.pointerState !== "up" && hit != null) {
      ...
      iMouseY =
        (canvasSizeY - (pointerY - /*toLocal*/ canvasesTop[hit])) *
        devicePixelRatio; // TODO: document
...
void main() {
  ...
  fragColor = vec4(outColor.xyz, 1.0);
      ...
      if (!gl.getShaderParameter(newFragmentShader, gl.COMPILE_STATUS)) {
        // TODO: get all other errors (e.g. link errors)
        ...
        editor.errorMarks = errors.map(({ line, messages }) => {
          ...
          const onMouseOver = (e: MouseEvent) => {
            ...
            tooltip.style.left = `${
              rect.right + 10 + /*toLocal*/ window.scrollX
            }px`;
    ...
    gl.uniform4f(
      ...
      iMouseX,
      iMouseY,
      inputs.pointerState === "up" ? 0 : 1,
      inputs.pointerState === "firstDown" ? 1 : 0
    );
